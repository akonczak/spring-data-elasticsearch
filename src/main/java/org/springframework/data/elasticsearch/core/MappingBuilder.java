...
class MappingBuilder {
	...
	//public static final String FIELD_PROPERTIES = "properties";//we can skip this with model
	...
	static Mappings buildMapping(Class clazz, String indexType, String idFieldName, String parentType) throws IOException {
		Mappings mappings = Mappings.builder().properties(new HashMap<>())
				.build();

		mapEntity(mappings.getProperties(), clazz, true, idFieldName, EMPTY, false, FieldType.Auto, null);
...
/*		Map<String,Object> mapping = new HashMap<>();
	...
	private static void mapEntity(Map<String,Property> properties, Class clazz, boolean isRootObject, String idFieldName,
		...
		Map<String, Property> scope = properties;
		...
		if (!isRootObject && (isAnyPropertyAnnotatedAsField(fields) || nestedOrObjectField)) {
			...
			if (nestedOrObjectField && FieldType.Nested == fieldType && fieldAnnotation.includeInParent()) {
				property.setIncludeInParent(fieldAnnotation.includeInParent());
			}
			scope.put(nestedObjectFieldName.toLowerCase(), property);
			scope = property.getProperties();
		}
		//add this to property object into properties field
		for (java.lang.reflect.Field field : fields) {
			...
			if (field.isAnnotationPresent(Mapping.class)) {
				...
				if (isNotBlank(mappingPath)) {
					...
					if (mappings.exists()) {
						throw new RuntimeException("Missing functionality to load mappings from file");
			...
			if (!isGeoPointField && !isCompletionField && isEntity(field) && isAnnotated(field)) {
				...
				mapEntity(scope, getFieldType(field), false, EMPTY, field.getName(), nestedOrObject, singleField.type(), field.getAnnotation(Field.class));
				if (nestedOrObject) {
					continue;
				}
			}
			...
			if (isGeoPointField) {
				applyGeoPointFieldMapping(scope, field);
			}

			if (isCompletionField) {
				CompletionField completionField = field.getAnnotation(CompletionField.class);
				applyCompletionFieldMapping(scope, field, completionField);
			}

			if (isRootObject && singleField != null && isIdField(field, idFieldName)) {
				applyDefaultIdFieldMapping(scope, field);
			} else if (multiField != null) {
				addMultiFieldMapping(scope, field, multiField, isNestedOrObjectField(field));
			} else if (singleField != null) {
				addSingleFieldMapping(scope, field, singleField, isNestedOrObjectField(field));
			}
		}
...
//		}
	...
	private static void applyGeoPointFieldMapping(Map<String, Property> scope, java.lang.reflect.Field field) throws IOException {
		scope.put(field.getName(), Property.builder().type(TYPE_VALUE_GEO_POINT).build());
	}

	private static void applyCompletionFieldMapping(Map<String, Property> scope, java.lang.reflect.Field field, CompletionField annotation) throws IOException {
		final Property property = Property.builder().type(TYPE_VALUE_COMPLETION).build();
		if (annotation != null) {
			property.setMaxInputLength(annotation.maxInputLength());
			property.setPreservePositionIncrements(annotation.preservePositionIncrements());
			property.setPreserveSeparators(annotation.preserveSeparators());
			if (isNotBlank(annotation.searchAnalyzer())) {
				property.setSearchAnalyzer(annotation.searchAnalyzer());
			}
			if (isNotBlank(annotation.analyzer())) {
				property.setAnalyzer(annotation.analyzer());
			}
		}
		scope.put(field.getName(), property);
	}

	private static void applyDefaultIdFieldMapping(Map<String, Property> scope, java.lang.reflect.Field field)
			throws IOException {
...
/*	private static void mapEntity(XContentBuilder xContentBuilder, Class clazz, boolean isRootObject, String idFieldName,
	...
	*//**
	 ...
	 * @throws IOException
	 */
	private static void addSingleFieldMapping(Map<String, Property> scope, java.lang.reflect.Field field,
											  Field fieldAnnotation, boolean nestedOrObjectField) throws IOException {

		final Property property = Property.builder().build();

		if(!nestedOrObjectField) {
			property.setStore(fieldAnnotation.store());
		}
		if(fieldAnnotation.fielddata()) {
			property.setFielddata(fieldAnnotation.fielddata());
		}

		if (FieldType.Auto != fieldAnnotation.type()) {
			property.setType(fieldAnnotation.type().name().toLowerCase());
			if (FieldType.Date == fieldAnnotation.type() && DateFormat.none != fieldAnnotation.format()) {
				property.setFormat(DateFormat.custom == fieldAnnotation.format()
						? fieldAnnotation.pattern() : fieldAnnotation.format().toString());
			}
		}
		if(!fieldAnnotation.index()) {
			property.setIndex(fieldAnnotation.index());
		}
		if (isNotBlank(fieldAnnotation.searchAnalyzer())) {
			property.setSearchAnalyzer(fieldAnnotation.searchAnalyzer());
		}
		if (isNotBlank(fieldAnnotation.analyzer())) {
			property.setAnalyzer(fieldAnnotation.analyzer());
		}
		scope.put(field.getName(), property);
	}
	/*
	...
	*//**
	 ...
	 * @throws IOException
	 */
	private static void addNestedFieldMapping(Property parentProperty, java.lang.reflect.Field field,
											  InnerField annotation) throws IOException {
		//TODO ako: this should be a Property not mappings
		final Property property = Property.builder().build();

		if (FieldType.Auto != annotation.type()) {
			property.setType(annotation.type().name().toLowerCase());
		}
		if(!annotation.index()) {
		...
		if (isNotBlank(annotation.indexAnalyzer())) {
			property.setAnalyzer(annotation.indexAnalyzer());
		}
		if (annotation.fielddata()) {
			property.setFielddata(annotation.fielddata());
		}
		parentProperty.getProperties().put(annotation.suffix(), property);
	}
	...
	*//**
	 ...
	 * @throws IOException
	 */
	private static void addMultiFieldMapping(Map<String, Property> scope, java.lang.reflect.Field field,
											 MultiField annotation, boolean nestedOrObjectField) throws IOException {
		final Property property = Property.builder()
				.type(annotation.mainField().type().toString())
				.build();

		for (InnerField innerField : annotation.otherFields()) {
			addNestedFieldMapping(property, field, innerField);
		}
		scope.put(field.getName(), property);
	}
